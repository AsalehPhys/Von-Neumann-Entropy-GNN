...
class EnhancedPhysicsGNN(nn.Module):
    """
    Deeper GNN with multi-task head: we output two values:
      [0]: log(S)/N
      [1]: S/N
    """
    def __init__(
        ...
        # Message Passing (GINEConv / TransformerConv)
        ...
        self.final_mlp = nn.Sequential(
            ...
            nn.SiLU(),
            nn.Dropout(dropout_p),
            nn.Linear(hidden_channels // 2, 2)  # output 2 values
        )
...
class MultiTaskLoss(nn.Module):
    ...
    def forward(self, preds, targets, system_size, subsystem_size):
        ...
        # 1) Physical scale aware loss on first head
...
def evaluate(model, loader, criterion, device, name='Eval'):
    ...
    for data in loader:
        ...
        # Compute loss only on non-[16,18] entries
        mask = (data.system_size.squeeze(-1) != 16) & (data.system_size.squeeze(-1) != 18)
        if mask.any():
            subsystem_size = data.nA.squeeze(-1)[mask]
            loss = criterion(
    ...
    predictions_abs = torch.cat(predictions_abs).numpy()
    predictions_over_n = torch.cat(predictions_over_n).numpy()
    ...
    for sz in sorted(np.unique(sizes)):
        ...
        size_metrics[sz_int] = {'mse': mse, 'mae': mae, 'mape': mape}
        
        # Format metrics string, using sz_int for display
...
def main():
    ...
    # We keep everything (including 16,18) in train_val_dataset, 
    # but the loss function will skip them.
    dataset_sizes = [dataset[i].system_size.item() for i in range(len(dataset))]
    train_val_subset = [dataset[i] for i in range(len(dataset))]
    
    # For final test, we still want a separate subset just to be consistent:
    test_subset = [
        dataset[i]
    ...
    if train_size == 0:
        logging.error("No data available for training split.")
        return
