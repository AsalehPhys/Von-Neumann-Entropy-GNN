...
# -----------------------------------------------------------
# EnhancedPhysicsGNN with Continuous Size Encoding
# -----------------------------------------------------------
class EnhancedPhysicsGNN(nn.Module):
    """
    Updated GNN with:
      - Continuous size encoding
      - Alternating GINEConv/TransformerConv
      - Additional global readouts
    Expects 10 global features: [Omega, Delta, Energy/size, RydExc/size, density, size, configEnt/size, relativeEnt, nA/size, nB/size]
    """
    def __init__(
        self,
        num_node_features,
        ...
        # Final MLP
        # Additional dimension for size encoding
    ...
    def forward(self, data):
        ...
        # Concatenate [set2set, global attention, global_feats, size_encoded]
...
# -----------------------------------------------------------
# Advanced Curriculum Sampler
# -----------------------------------------------------------
def get_curriculum_sampler(dataset, epoch, max_epochs):
    """
    Progressively up-weights larger system sizes.
    At epoch=0, strongly favors smaller systems. 
    By epoch ~ 0.7*max_epochs, weighting is uniform or balanced.
    """
    ...
    # For each sample, weight depends on size
    # smaller sizes start with weight=1.0, bigger sizes gradually get more weight
    weights = []
    for size in system_sizes:
        # Normalize size to [0, 1]
        size_ratio = (size - 4) / (max_size - 4 + 1e-6)
        # Weight is 1 for small sizes plus a factor that grows with progress
        if size <= 4:
            weight = 1.0
        else:
            weight = 1.0 + progress * size_ratio
...
def evaluate(model, loader, criterion, device, name='Eval'):
    ...
    for data in loader:
        ...
        # Recover predicted S from log(S)/N:
        pred_entropy = torch.exp(out * data.system_size.squeeze(-1))
        predictions.append(pred_entropy.cpu())
...
def main():
    ...
    criterion = PhysicalScaleAwareLoss(physics_weight=0.3)
    ...
    for epoch in range(CONFIG['num_epochs']):
        ...
        # Optional: check size-14 performance each epoch
