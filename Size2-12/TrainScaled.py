...
CONFIG = {
    ...
    'patience': 75,
    'random_seed': 42,
    'best_model_path': 'best_model_rung1_6_scaled.pth',
    'dropout_p': 0.3,
...
def evaluate(model, loader, criterion, device, rank, name='Eval'):
    ...
    # We'll collect predictions, targets, and system sizes here
    all_preds_scaled = []
    all_targets = []
    all_sizes = []
    ...
    for data in loader:
        data = data.to(device)
        pred_s = model(data)   # This is \hat{S/N}

        targets = data.y.squeeze()  # This is S
        system_size = data.system_size.squeeze(-1)
        subsystem_size = data.nA.squeeze(-1)
        ...
        # Store them on CPU for later metrics
        all_preds_scaled.append(pred_s.cpu())
        all_targets.append(targets.cpu())
        all_sizes.append(system_size.cpu())
    ...
    if rank == 0:
        # Concatenate all the predictions, targets, and system sizes
        all_preds_scaled = torch.cat(all_preds_scaled).numpy()  # \hat{S/N}
        all_targets = torch.cat(all_targets).numpy()            # raw S
        all_sizes = torch.cat(all_sizes).numpy()

        # Convert predicted S/N back to predicted S
        predicted_S = all_preds_scaled * all_sizes
        real_S = all_targets

        # Now compute MSE, MAE, etc. in terms of raw S
        mse_abs = mean_squared_error(real_S, predicted_S)
        mae_abs = mean_absolute_error(real_S, predicted_S)
        # If you want standard MAPE, you'd typically divide by real_S
        # but if you want your own variation, that's fine
        mape_abs = np.mean(np.abs((predicted_S - real_S) / (real_S + 1e-10))) * 100
