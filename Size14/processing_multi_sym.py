...
CONFIG = {
    'data_paths': [
        'Rydbergsize10Delta3.5.parquet',
    ],
    'processed_dir': './processed_size10_2.5Delta',
    'processed_file_name': 'data.pt',
    'random_seed': 42,
    'chunk_size': 50,  # Size of chunks for sequential processing
    ...
    'batch_size': 50,  # Increased batch size for better parallelism
    'timeout': 600,  # 10-minute timeout per batch
    'distance_cutoff': 6,
    'parallel_processing': True,  # Enable parallel processing
    'read_entropy_from_file': True  # New flag to toggle between reading or calculating entropy values
}
...
def get_indices_from_mask(mask):
    """Convert a binary mask to a list of indices"""
    return [i for i, val in enumerate(mask) if val == 1]
...
def process_single_row(row_data):
    try:
        ...
        # Determine whether to read entropy values from file or calculate them
        if CONFIG['read_entropy_from_file']:
            # Check if the necessary columns exist
            if all(col in row_data for col in ['Von_Neumann_Entropy', 'Classical_MI', 'Subsystem_Mask']):
                vn_entropy = float(row_data['Von_Neumann_Entropy'])
                classical_mi = float(row_data['Classical_MI'])
                mask = np.array(row_data['Subsystem_Mask'], dtype=np.int32)
                A_indices = get_indices_from_mask(mask)
            else:
                logging.warning("Required columns not found in parquet file. Calculating values instead.")
...
def load_and_process_data_in_batches():
    ...
    for path in CONFIG['data_paths']:
        ...
        logging.info(f"Reading entropy/MI values from file: {CONFIG['read_entropy_from_file']}")
