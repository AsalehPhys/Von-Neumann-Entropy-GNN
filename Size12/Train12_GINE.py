...
class ExperimentalGNN(nn.Module):
    def __init__(
        ...
        self.feature_indices = {
            ...
            'geometric_features': slice(4, 11), # Geometric features
        }

        # Node feature transformation
        # Input: positions (2) + Rydberg probability (1) + mask (1) + geometric features (7) + quantum features (12) = 23 features
        self.node_encoder = nn.Sequential(
            nn.Linear(11, hidden_channels),  # 23 input features
            nn.LayerNorm(hidden_channels),
            nn.SiLU()
        )

        # Edge feature transformation
        # Input: angle (1) + correlation values (1) + distance (1) = 3 features
        self.edge_encoder = nn.Sequential(
            nn.Linear(3, hidden_channels // 2),  # 3 input features
    ...
    def forward(self, data):
        ...
        node_features = torch.cat([
            ...
            x[:, self.feature_indices['geometric_features']],  # geometric features
        ], dim=1)

        # Select all edge features
        edge_features = torch.stack([
            edge_attr[:, 0],  # angle
            edge_attr[:, 1],  # correlation values
