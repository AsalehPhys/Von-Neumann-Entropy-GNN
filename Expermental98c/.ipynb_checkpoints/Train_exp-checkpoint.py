...
CONFIG = {
    ...
    'patience': 75,
    'random_seed': 42,
    'best_model_path': 'best_experimental_gnn_98c.pth',
    'dropout_p': 0.4,
...
class ExperimentalGNN(nn.Module):
    ...
    def __init__(
        self,
        num_node_features,  # Should be 4: [x, y, rydb_exp_val, mask]
        edge_attr_dim,      # Should be 3: [angle, correlation, normalized_dist]
        hidden_channels=256,
        num_layers=10,
        dropout_p=0.4
    ):
        ...
        # Message Passing layers remain the same
        ...
        # Global transform - now using only system size and partition sizes
        ...
        # Size encoding remains the same
        ...
        combined_in_dim = (2 * hidden_channels) + (2 * hidden_channels) + (hidden_channels // 2)
    ...
    def forward(self, data):
        ...
        # Global features - using only system size and partition information
        system_size = data.system_size.squeeze(-1)
        nA = data.nA.squeeze(-1)
        nB = data.nB.squeeze(-1)

        global_feats = torch.stack([
            nA / system_size,          # Normalized partition A size
            nB / system_size,          # Normalized partition B size
