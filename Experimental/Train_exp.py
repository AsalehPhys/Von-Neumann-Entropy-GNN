...
CONFIG = {
    ...
    'patience': 75,
    'random_seed': 42,
    'best_model_path': 'best_experimental_gnn.pth',
    'dropout_p': 0.4,
...
class PhysicalScaleAwareLoss(nn.Module):
    ...
    def get_entropy_bounds(self, system_size, subsystem_size, mutual_info):
        """Get bounds on entropy using mutual information as lower bound"""
        # Use mutual information as lower bound instead of zero
        lower_bound = mutual_info
    ...
    def forward(self, pred_log_s_over_n, target, system_size, subsystem_size, mutual_info):
        # Convert prediction to actual entropy (S_pred)
        pred_entropy = torch.exp(pred_log_s_over_n * system_size)

        # Physical bounds using MI
        lower_bound, upper_bound = self.get_entropy_bounds(system_size, subsystem_size, mutual_info)
...
class MultiTaskLoss(nn.Module):
    ...
    def forward(self, preds, targets, system_size, subsystem_size, mutual_info):
        pred_log_s_over_n = preds[:, 0]
        pred_s_over_n = preds[:, 1]

        # Physical scale aware loss with MI bound
...
def train_epoch(model, loader, optimizer, criterion, device, clip_grad=None):
    ...
    for data in loader:
        ...
        # Forward
        preds = model(data)  # shape (batch, 2)
        ...
        mutual_info_ = data.mutual_info.squeeze()[mask]
        preds_ = preds[mask]

        loss = criterion(preds_, targets_, system_size_, subsystem_size, mutual_info_)
        loss.backward()
...
def evaluate(model, loader, criterion, device, name='Eval'):
    ...
    for data in loader:
        ...
        # Keep everything as tensors until final conversion
        ...
        mutual_infos.append(data.mutual_info.squeeze().cpu())

    # Count total non-[16,18] samples for proper loss averaging
    ...
    # Concatenate tensors first, then convert to numpy if needed
    predictions_abs = torch.cat(predictions_abs)
    predictions_over_n = torch.cat(predictions_over_n)
    targets = torch.cat(targets)
    sizes = torch.cat(sizes)
    mutual_infos = torch.cat(mutual_infos)

    # Check MI violations while still tensors
    violations = predictions_abs < mutual_infos
    violation_rate = violations.float().mean().item()
    
    # Average violation magnitude
    violation_magnitude = torch.where(
        violations,
        mutual_infos - predictions_abs,
        torch.zeros_like(predictions_abs)
    ).mean().item()

    # Now convert to numpy for metrics computation
    predictions_abs = predictions_abs.numpy()
    predictions_over_n = predictions_over_n.numpy()
    targets = targets.numpy()
    sizes = sizes.numpy()
    mutual_infos = mutual_infos.numpy()

    # Compute metrics per size
    metrics_str = []
    size_metrics = {}
    for sz in sorted(np.unique(sizes)):
        sz_int = int(sz)  # Convert to integer for metrics storage and display
        mask = (sizes == sz)
        size_preds = predictions_abs[mask]
        size_targets = targets[mask]
        size_mi = mutual_infos[mask]
        ...
        # MI violations per size
        size_violations = (size_preds < size_mi).mean() * 100
        
        size_metrics[sz_int] = {
            'mse': mse, 
            'mae': mae, 
            'mape': mape,
            'mi_violation_rate': size_violations
        }
        
        metrics_line = f"Size {sz_int:2}: MSE={mse:.2e} MAE={mae:.2e} MAPE={mape:.1f}% MI-Viol={size_violations:.1f}%"
        if sz_int in [16, 18]:
    ...
    for line in metrics_str:
        logging.info(f"  {line}")
    logging.info(f"Overall MI Lower Bound Violations:")
    logging.info(f"  Rate: {violation_rate*100:.2f}%")
    logging.info(f"  Mean Magnitude: {violation_magnitude:.4f}")
    ...
    return {
        ...
        'mi_violation_rate': violation_rate,
        'mi_violation_magnitude': violation_magnitude
    }
# -----------------------------------------------------------
# Main
# -----------------------------------------------------------
